...
export class DoctorComponent {
  ...
  getAllData() {
    this.doctorService.getAllDoctors().subscribe((response) => {
      ...
      this.dataSource = this.doctorDetails;
    });
  }
  saveCustomer() {
    if (this.availability?.value === 'Yes') {
      this.doctorDto.availability = true;
    } else {
      this.doctorDto.availability = false;
    }
    ...
    this.doctorService.doctorRegistrarion(this.doctorDto).subscribe(
      (response) => {
